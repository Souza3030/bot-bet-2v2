...
export async function handleDuoBetPayButton(interaction: ButtonInteraction): Promise<void> {
  ...
  if (!leg) {
    await interaction.reply({
      content: "⚠️ Você não faz parte desta dupla.",
      ephemeral: true,
    });
    return;
  }
  ...
  if (leg.status !== "aguardando_pagamento") {
    await interaction.reply({
      content:
        leg.status === "paga_aguardando_parceiro" || leg.status === "na_fila"
          ? "✅ Você já pagou sua parte! Só falta seu parceiro (ou vocês já estão na fila)."
