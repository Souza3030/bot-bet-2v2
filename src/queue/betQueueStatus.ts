...
function buildQueueStatusEmbed(waitingTeams: WaitingDuoTeam[]): EmbedBuilder {
  const lines =
    waitingTeams.length > 0
      ? waitingTeams.map(
          (team) => `${formatTeam(team)} — **R$ ${team.amount.toFixed(2)}/jogador** — aguardando adversário`
        )
      : ["Nenhuma dupla esperando no momento."];

  return new EmbedBuilder()
    .setColor(waitingTeams.length > 0 ? 0xf1c40f : 0x5865f2)
    .setTitle(`🏐 Fila Apostada Duo (2v2) — ${waitingTeams.length} dupla(s) esperando`)
    .setDescription(lines.join("\n"))
    .setFooter({ text: "Use /apostar-dupla @parceiro para convidar alguém e entrar na fila." })
    .setTimestamp();
}
...
export async function markQueueAsCheckingIn(guild: Guild, teamA: WaitingDuoTeam, teamB: WaitingDuoTeam): Promise<void> {
  const embed = new EmbedBuilder()
    .setColor(0xe67e22)
    .setTitle("🏐 Fila Apostada Duo (2v2) — Duplas pareadas!")
    .setDescription(
      [
        `**Dupla A:** ${formatTeam(teamA)}`,
        `**Dupla B:** ${formatTeam(teamB)}`,
        `💰 Pote: **R$ ${((teamA.amount + teamB.amount) * 2).toFixed(2)}**`,
        "",
        "Os 4 jogadores estão confirmando presença para a partida.",
      ].join("\n")
    )
...
export async function markMatchCreated(guild: Guild, matchChannelId: string): Promise<void> {
  await editStatus(
    ...
    new EmbedBuilder()
      ...
      .setDescription(`As duplas foram pareadas e a partida foi criada em <#${matchChannelId}>.`)
      .setFooter({ text: "Use /apostar-dupla novamente para entrar em uma nova fila." })
      .setTimestamp()
  );
}
