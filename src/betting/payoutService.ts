...
/** Reembolsa as pernas de um "time" (na fila solo, sempre 1 perna). */
export async function refundTeam(legs: BetLeg[], guild: Guild, reason: string): Promise<void> {
  await Promise.all(legs.map((leg) => refundLeg(leg, guild, reason)));
}

/**
 * Paga o(s) jogador(es) do lado vencedor, dividindo o pote (todas as
 * apostas, menos a taxa da casa) igualmente entre eles. Na fila solo
 * (1x1) o lado vencedor tem 1 perna só, então o vencedor leva o pote
 * inteiro. Cada payout é tentado individualmente — se um falhar, o outro
 * não é afetado, e o que falhou cai no fallback manual para a Staff.
 */
export async function payoutWinningTeam(
  ...
  const perWinner = Math.round((totalPayout / winnerLegs.length) * 100) / 100;
  ...
  await Promise.all(
    winnerLegs.map(async (leg) => {
      ...
      try {
        ...
        await user
          ?.send(
            `🏆 Você venceu! **R$ ${perWinner.toFixed(2)}** foram enviados via PIX para sua chave ` +
