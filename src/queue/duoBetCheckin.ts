...
function buildCheckInEmbed(session: DuoBetCheckInSession): EmbedBuilder {
  const pot = session.teamA.amount + session.teamB.amount;
  ...
  const teamLine = (team: WaitingDuoTeam, label: string) =>
    team.memberIds
      ...
      .join(" • ")
      .replace(/^/, `**${label} (R$ ${team.amount.toFixed(2)}):** `);
  ...
  return new EmbedBuilder()
    ...
    .setDescription(
      [
        `💰 **Pote da partida: R$ ${pot.toFixed(2)}** (vai inteiro pro vencedor, descontada a taxa da casa se houver).`,
        `Vocês têm **${config.queue.checkInTimeoutMs / 1000} segundos** para confirmar presença.`,
        "",
        teamLine(session.teamA, "Jogador A"),
        teamLine(session.teamB, "Jogador B"),
        "",
        pending.length > 0
 ...
 * - Os 2 jogadores confirmam: cria a partida normalmente.
 * - Um jogador confirma e o outro não: quem confirmou volta pra fila
 *   (mantém o dinheiro em jogo); quem não confirmou é reembolsado.
 * - Nenhum confirma: os 2 são reembolsados.
 */
async function resolveCheckIn(sessionId: string, guild: Guild): Promise<void> {
  ...
  for (const team of discarded) {
    ...
    if (validLegs.length > 0) {
      await refundTeam(validLegs, guild, "jogador não confirmou presença a tempo");
    }
  }
            ...
            .setDescription(
              [
                discarded
                  .map(
                    (team) =>
                      `${team.memberIds.map((id) => `<@${id}>`).join(" & ")} não confirmou presença a tempo e foi reembolsado.`
                  )
                  .join("\n"),
                requeued.length > 0
                  ? `Quem confirmou (${requeued
...
async function refundBothTeams(
  ...
  for (const team of [teamA, teamB]) {
    ...
    const validLegs = legs.filter((l): l is NonNullable<typeof l> => l !== null);
    if (validLegs.length > 0) {
      await refundTeam(validLegs, guild, reason);
    }
  }
}
